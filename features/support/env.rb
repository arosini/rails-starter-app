...
at_exit do
  ...
  if max_scenarios > 1
    puts
	  puts "------------- Top #{max_scenarios} slowest scenarios (out of #{scenario_times.size} scenarios run) -------------"
	  sorted_times = scenario_times.sort { |a, b| b[1] <=> a[1] }
	  sorted_times[0..max_scenarios - 1].each_with_index do |(key,value),index|
	    puts "#{index + 1}) #{value.round(2)}s - #{key}"
	  end
    puts
  end
end